...
def extraer_modulos_basic(ruta_odb, dir_salida=None):
    ...
    # Construir nombre de archivo con doble extensión
    nombre_archivo = f"{ruta_odb.stem}_modulos_consolidados.xml.txt"
    ruta_salida_completa = str(dir_salida / nombre_archivo)
    
    print(f"[DEBUG] Archivo de salida sera: {ruta_salida_completa}")
    ...
    with zipfile.ZipFile(ruta_odb, 'r') as zip_ref:
        ...
        for biblioteca in sorted(bibliotecas):
            ...
            print(f"  Modulos encontrados: {len(modulos_xml)}")
    ...
    generar_xml_consolidado(modulos_por_biblioteca, ruta_salida_completa, ruta_odb.name)
...
def generar_xml_consolidado(modulos_por_biblioteca, ruta_salida, nombre_odb):
    ...
    Solo genera el archivo .xml.txt (no el .xml).
    """
    ...
    for nombre_biblioteca, modulos in modulos_por_biblioteca.items():
        ...
        for modulo in modulos:
            ...
            # Añadir atributos del módulo original (limpiar namespaces)
    ...
    # Escribir el archivo
    print(f"[DEBUG] Escribiendo en: {ruta_salida}")
...
if __name__ == '__main__':
    ...
    try:
        print("=" * 60)
        print("CONSOLIDADOR DE MODULOS BASIC")
        print("=" * 60)
